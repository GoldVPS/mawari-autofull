...
choose(){
  ...
  echo "3) Run (delegate-only, auto-discover tokenIds)"
...
main(){
  while true; do
    ...
    case "$c" in
      ...
      3) banner "Run delegate-only"; bash "$REPO_DIR/scripts/run_delegate_only.sh" ;;
      4) banner "Logs (Ctrl+C to exit)"; docker logs -f --tail=200 "$CONTAINER" || echo "Container not found." ;;
